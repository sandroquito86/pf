...
class Categoria(models.Model):
    ...
    nombre_equipos = fields.One2many(string='Tipos', comodel_name='pg_equipos.nombre_equipo', inverse_name='categoria_id',)  
...
class NombreEquipo(models.Model):
    ...
    componente_ids = fields.Many2many(string='Componentes', comodel_name='asset.catalogo_componente', relation='tipo_catalogo_componente_rel', column1='nombre_equipo', column2='componente_id',
                                      domain="[('categoria_id','=',categoria_id)]")
    
    active = fields.Boolean(string='Activo/Inactivo', default='True' )
    
    _sql_constraints = [('name_unique', 'UNIQUE(categoria_id,name)', "Tipo de activo ingresado ya existe en esta categoría!!")]    
